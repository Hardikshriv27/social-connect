...
import { FacebookIcon, InstagramIcon, YoutubeIcon } from "@/components/social/PlatformIcons";
...
const PLATFORMS = [
  ...
  { id: "YOUTUBE", label: "YouTube", icon: YoutubeIcon, color: "text-[#FF0000]" },
] as const;
...
export default function CreatePostPage() {
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setMedia((prev) => [...prev, { url: data.url, name: file.name }]);
    } catch {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-8">
        <h1 className="text-xl font-semibold tracking-tight text-ink">Create post</h1>
        <p className="mt-1 text-[13.5px] text-ink-muted">
          Write your content, attach media, and publish or schedule it.
        </p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6 rounded-lg border border-line p-6">
        <div>
        ...
        <div>
          ...
          <p className="mb-2.5 text-[12.5px] text-ink-muted">
            Select where this post is intended for. Publishing requires a connected account.
          </p>
          <div className="flex flex-wrap gap-2">
            {PLATFORMS.map(({ id, label, icon: Icon, color }) => {
              ...
              return (
                ...
                >
                  <Icon className={cn("h-4 w-4", isSelected ? "text-brand" : color)} />
                  {label}
                </button>
              );
            })}
